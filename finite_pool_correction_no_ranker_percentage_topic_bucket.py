...
use_ranker = 'True'
iter_sampling = 'True'
correction = 'False' #'SAL' can be ['SAL', 'CAL', 'SPL']
...
topicSkipList = [1000] # Dummy Value
topicSkippedbyRanker = []
...
base_address = "/home/nahid/UT_research/clueweb12/ranker_result/"
...
for test_size in test_size_set:
    ...
    for fold in xrange(1,2):
        ...
        for topic in xrange(start_topic,end_topic):
            ...
            prevelance_ratio = (label_Ones*1.0)/len(docList)
            #if prevelance_ratio > 0.05:
            #    continue
            ...
            if use_ranker == True:
                ...
                if seed_one_counter == 0:
                    ...
                    while seed_one_counter < 2:
                        if ask_for_label > math.ceil(len(X)*0.09):
                            topicSkippedbyRanker.append(topic)
                            topicSkipFlag = 'True'
                            break
                        documentNumber = seed_list[seed_counter]
...
print "Topic Skipped By Ranker:"
for topic in topicSkippedbyRanker:
    print topic
